  ...
  return <div className="learn-scene-support" aria-label="Tutor teaching">
    {fallbackBlocks.map((block) => {
      ...
      return <section className={`learn-scene-block learn-scene-block-${block.kind}`} key={block.id}>
        <p className="learn-scene-block-label">{block.label}</p>
...
export function LearnView({ note, documentId, onBack }: { note: SectionNote; documentId: number | null | undefined; onBack: () => void }) {
  ...
  async function start() {
    ...
    try { const created = await api.createLearnSession(documentId, { goal, familiarity }); sessionRef.current = created; setSession(created); setFocusMode(true); setAnswer(""); setSelectedOption(null); setOrderedIds(created.step?.items?.map((item) => item.id) ?? []); setStructuredAnswers({}); setHint(null) }
