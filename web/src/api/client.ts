...
export type LearnScene = { id: string; revision: number; objectiveId: string; objective: string; targetConcepts: string[]; pedagogicalGoal: string; tutorHypothesis: string; strategy: string; scaffoldLevel: string; blocks: LearnSceneBlock[]; evidenceTargets: string[]; sourceSectionIds: string[]; sourceBlockIds: string[]; visualState: { stage?: number; highlight?: string | null; [key: string]: unknown }; completionCondition: string; responseStepId?: string | null }
...
export type AskLucentResponse = { answer: string; scope: "IN_SCOPE_SOURCE" | "IN_SCOPE_CURRENT_CONCEPT" | "IN_SCOPE_PREREQUISITE" | "OUT_OF_SCOPE"; sourceSectionIds: string[]; sourceBlockIds: string[]; tool: string; visualAction?: { type: string; stepId?: string; stage?: number; nodeId?: string } | null; scenePatch?: LearnScene | null }
