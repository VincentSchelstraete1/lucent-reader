...
export type LearnVisualSpec = { type: "diagram" | "process" | "process_flow" | "labeled_diagram" | "relationship_map" | "comparison" | "sequence" | "labeling" | "ordering" | "prediction" | "staged_visual" | "step_through"; title: string; purpose: string; nodes: Array<{ id: string; label: string; detail?: string | null; group?: string | null }>; edges: Array<{ source: string; target: string; label?: string | null }>; stages: Array<{ title?: string; explanation?: string; activeNodeIds?: string[] }>; answerId?: string | null; sourceSectionIds?: string[]; sourceBlockIds?: string[] }
export type LearnStepView = { id: string; type: string; title: string; prompt?: string | null; content?: string | null; options: LearnOption[]; items: LearnOption[]; visualSpec?: LearnVisualSpec | null; visualRef?: { sectionId?: string; componentIndex?: number } | null; sectionId?: string | null; componentIndex?: number | null; hintsAvailable: number }
