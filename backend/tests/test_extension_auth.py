...
def _authorization_code(user, code="one-time-code"):
    with TestSessionLocal() as db:
        db.add(ExtensionAuthorizationCode(
            user_id=user.id, code_hash=token_hash(code), code_challenge=pkce_challenge(VERIFIER),
...
def test_authorization_code_pkce_and_single_use(monkeypatch):
    ...
    wrong = client.post("/auth/extension/token", json={"code": code, "code_verifier": "wrong", "redirect_uri": REDIRECT})
    assert wrong.status_code == 400
    good = client.post("/auth/extension/token", json={"code": code, "code_verifier": VERIFIER, "redirect_uri": REDIRECT})
    assert good.status_code == 200
    assert client.post("/auth/extension/token", json={"code": code, "code_verifier": VERIFIER, "redirect_uri": REDIRECT}).status_code == 400
...
def test_access_expiry_refresh_rotation_and_reuse_revokes_family(monkeypatch):
    ...
    client = TestClient(app)
    issued = client.post("/auth/extension/token", json={"code": code, "code_verifier": VERIFIER, "redirect_uri": REDIRECT}).json()
