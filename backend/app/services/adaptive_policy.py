...
def prerequisite_ids(objective: dict, concepts: list[dict]) -> list[str]:
    ...
    if declared:
        # A declared source-backed dependency is a possible branch, not proof
        # that remediation is needed.  Never interrupt for a prerequisite the
        # learner has already demonstrated.  Unseen dependencies remain valid
        # because the runtime initializes their evidence on branch entry.
        known = {str(item.get("conceptId")): item for item in concepts}
        return [
            str(item) for item in declared
            if known.get(str(item), {}).get("state") != "DEMONSTRATED"
        ]
