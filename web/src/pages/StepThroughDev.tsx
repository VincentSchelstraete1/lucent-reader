...
import { gramSchmidtGolden, photosynthesisGolden } from "../learning/experiences/goldenExamples"
...
export function StepThroughDev() {
  ...
  return <section className="page step-through-dev">
    ...
    <div className="step-through-toolbar">
      <label>Source fixture<select value={selected} onChange={(event) => choose(event.target.value)}><option value="photosynthesis-poc">photosynthesis-poc · semantic visual</option>{fixtures.map((item) => <option key={item.name} value={item.name}>{item.name}{item.replay_available ? " · replay available" : ""}</option>)}</select></label>
    ...
    {!result && selected === "photosynthesis-poc" && <section className="step-through-result"><p className="step-through-muted">Mechanism visual proof of concept: semantic regions, membrane boundary, particles, flows, and stage-specific narration.</p><h2>Light-dependent reactions</h2><StepThroughMechanism data={photosynthesisGolden} /></section>}
