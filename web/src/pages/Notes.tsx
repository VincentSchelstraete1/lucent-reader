...
function LearningSceneView({ session, note }: { session: LearnSession; note: SectionNote }) {
...
export function LearnView({ note, documentId, onBack }: { note: SectionNote; documentId: number | null | undefined; onBack: () => void }) {
  ...
  async function start() {
    ...
    try { const created = await api.createLearnSession(documentId, { goal, familiarity }); sessionRef.current = created; setSession(created); setFocusMode(true); setAnswer(""); setSelectedOption(null); setOrderedIds(created.scene?.blocks.find((block) => block.kind === "practice")?.step?.items?.map((item) => item.id) ?? []); setStructuredAnswers({}); setHint(null) }
  ...
  const step = session.scene?.blocks.find((block) => block.kind === "practice")?.step
  const sceneTeaching = session.scene?.blocks.filter((block) => block.kind !== "practice") ?? []
  if (!step) return <section className={`learn-workspace learn-session ${focusMode ? "learn-focus-mode" : ""}`} aria-labelledby="learn-heading"><div className="learn-session-top"><button className="learn-back" type="button" onClick={onBack}>← Back to notes</button><span>Objective {session.objectiveIndex + 1} of {session.objectiveCount}</span><button className="learn-focus-toggle" type="button" onClick={() => setFocusMode((value) => !value)}>{focusMode ? "Exit focus" : "Focus learning"}</button></div><p className="note-kicker">Watch</p><h2 id="learn-heading">{session.scene?.objective ?? note.title}</h2>{sceneTeaching.map((block) => <div className="learn-teaching-block" key={block.id}><h3>{block.title ?? block.label}</h3>{block.content && <p>{block.content}</p>}</div>)}<div className="learn-step-actions"><button className="btn" type="button" onClick={stop}>Stop for now</button><button className="btn btn-primary" type="button" onClick={() => respond()}>Continue</button></div></section>
