...
function FlashcardsView({ notes, onBack }: { notes: SectionNote[]; onBack: () => void }) {
  ...
  const flip = () => setRevealed((value) => !value)
  return <section className={`learn-workspace flashcards-workspace${focusMode ? " learn-focus-mode flashcards-focus-mode" : ""}`} aria-labelledby="flashcards-heading" onKeyDown={(event) => { if (event.key === "Escape") setFocusMode(false) }}><div className="flashcards-topbar"><button className="learn-back" type="button" onClick={onBack}>← Back to notes</button><button className="learn-focus-toggle" type="button" aria-pressed={focusMode} onClick={() => setFocusMode((value) => !value)}>{focusMode ? "Exit focus" : "Expand"}</button></div><p className="note-kicker">Recall practice</p><h2 id="flashcards-heading">Flashcards</h2><p className="learn-context">{index + 1} of {cards.length} · {card.section}</p><article className={`flashcard${revealed ? " is-revealed" : ""}`} role="button" tabIndex={0} aria-live="polite" aria-label={revealed ? "Flashcard answer. Click to show the question." : "Flashcard question. Click to reveal the answer."} onClick={flip} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); flip() } }}>{revealed ? <div className="flashcard-answer"><span>Answer</span><strong>{card.answer}</strong></div> : <p>{card.prompt}</p>}</article><p className="flashcard-instruction">Click the card to {revealed ? "see the question" : "reveal the answer"}.</p><div className="step-controls"><button type="button" onClick={() => { setIndex((value) => Math.max(0, value - 1)); setRevealed(false) }} disabled={index === 0}>Previous card</button><button type="button" onClick={flip}>{revealed ? "Show question" : "Reveal answer"}</button><button type="button" onClick={() => { setIndex((value) => (value + 1) % cards.length); setRevealed(false) }} disabled={!revealed}>Next card</button></div></section>
}
