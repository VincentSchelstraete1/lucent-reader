...
from app.semantic import SemanticGenerator, plain_text_fallback
...
def route_canvas_text(
    ...
    block = SimpleNamespace(id="learning-canvas", text=request.text.strip(), title=None)
    decision: RepresentationDecision = route_learning_block_hybrid(block, classifier)
    try:
        learning_object = semantic_generator.generate(block, decision)
    except Exception:
