...
function SectionNotesInspection({ notes }: { notes: NonNullable<DocumentIngestionResult["section_notes"]> }) {
  ...
  return <section className={styles.markdownSection} aria-labelledby="section-notes-heading"><details open><summary id="section-notes-heading">Coherent section notes ({notes.length})</summary>{notes.map(note => <article key={note.id} className={styles.pageCard}><h3>{note.title}</h3><p><strong>Big idea:</strong> {note.bigIdea}</p>{note.components.map(component => <div key={`${note.id}-${component.title}`}><h4>{component.title}</h4><p><small>{component.kind} · source blocks: {component.sourceBlockIds.join(", ")}</small></p>{component.learningObject ? <LearningObjectRenderer object={component.learningObject} /> : component.kind === "flow" || component.kind === "relationship_map" ? <ol>{component.nodes.map(node => <li key={String(node.id)}>{String(node.label)}{component.edges.find(edge => String(edge.source ?? edge.from) === String(node.id)) && <small> → {String(component.edges.find(edge => String(edge.source ?? edge.from) === String(node.id))?.relation ?? component.edges.find(edge => String(edge.source ?? edge.from) === String(node.id))?.label)}</small>}</li>)}</ol> : component.kind === "structure" && component.root ? <pre>{JSON.stringify(component.root, null, 2)}</pre> : component.kind === "key_definition" ? <p><strong>{component.term}</strong>: {component.definition}</p> : component.kind === "worked_example" || component.kind === "equation" ? <><p>{component.problem || component.equation || component.text}</p><ol>{component.steps.map(step => <li key={String(step.label ?? step)}>{String(step.label ?? step)}</li>)}</ol>{component.result && <p><strong>Result:</strong> {component.result}</p>}{component.interpretation && <p><strong>Meaning:</strong> {component.interpretation}</p>}</> : <p>{component.text || component.takeaway}</p>}</div>)}<h4>Takeaways</h4><ul>{note.keyTakeaways.map(item => <li key={item}>{item}</li>)}</ul></article>)}</details></section>
}
